...
function moveStruct(_damage=0, _button = noone, _animation, _effect, _animationStart, _animationEnd, _sound) constructor{
	...
	owner = noone
	sound = _sound
	getOpponent = function(){return owner == global.mewtwo ? global.active_starter : global.mewtwo}
}
...
function animationStartProjectile(){
	if(owner!= global.mewtwo) {
		...
		instance_create_depth(global.mewtwo.x, global.mewtwo.y, owner.depth + 1 - 2 * (owner == global.mewtwo), animation, {move : other})
	}
...
function animationStartBasic() {
	instance_create_depth(0, 0, owner.depth + 1 - 2 * (owner == global.mewtwo), animation, {move : other})
	audio_play_sound(sound, 0, 0)
	audio_play_sound(owner.sound, 0, 0)
}
...
function switchTurn() {
	global.turn = global.turn == TURNS.PLAYER ? TURNS.MEWTWO : TURNS.PLAYER
}
